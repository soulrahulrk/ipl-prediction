...
## Latest Updates: 2026 Active Team + Squad Refresh

### What Changed
- Frontend is now high-contrast for better visibility on all displays.
- Team selectors are limited to active IPL teams for 2026 context.
- Old/retired franchises are excluded from the Streamlit team dropdowns.
- Preprocessing now writes:
  - `data/processed/active_teams_2026.csv`
  - `data/processed/team_player_pool_2026.csv`
- Training now filters to:
  - active IPL teams only
  - recent seasons (rolling window, currently 2024-2026)

### Why This Matters
- Prevents retired teams from polluting predictions.
- Keeps squad/player options aligned with recent IPL seasons.
- Makes model behavior more relevant to current tournament context.
- Improves readability and usability of the app for real match usage.
...
- Applies active-team + recent-season filter before training
